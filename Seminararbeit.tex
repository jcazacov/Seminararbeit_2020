...
Diese Tatsache hat im Deutschen keinen eigenen Namen, ist jedoch im englischsprachigen Raum als das "Little Bézout's theorem" bekannt. %quelle nötig
...
Dabei wird von einem Vektor, der im Ursprung beginnt und an dem zu beschreibenden Punkt endet, dessen Winkel $\varphi$ zur Realachse und die Länge dieses Vektors $r$ angegeben. Diese Länge $r$ wird der Betrag $r=|z|$ der komplexen Zahl $z$ genannt.\footnote{Ansorge, Rainer; Oberle, Hans Joachim; Rothe, Kai; Sonar, Thomas, Mathematik für Ingenieure, Hamburg $2010^4$ S.$35$}
...
annähern, wobei bei steigender Anzahl $n$ der Polynomglieder die Genauigkeit der Annäherung steigt.\footnote{Vgl. Bronstein, Ilja Nikolajewitsch; Mühlig, Heiner; Musiol, Gerhart; Semendjajew, Konstantin Adolfowitsch: Taschenbuch der Mathematik, Haan-Gruiten $2016^{10}$ S. $455$}
...
Bedenken wir, dass $i^2=-1$, $i^3=-i$ und $i^4=1$, entsteht durch Umformungen und Umgruppierungen:
...
\end{equation}
mit $r=|z|$ und $\varphi=arg(z)$."\footnote{{Schmidt, Harald: Elementare Technomathematik, Amberg $2018$, S. $299$}}\\
...
Wenn wir in diese Formel die eulersche Formel \eqref{euler} einsetzen, entsteht die Formel von Moivre:
\begin{equation}
...
\footnote{Bronstein, Ilja Nikolajewitsch; Mühlig, Heiner; Musiol, Gerhart; Semendjajew, Konstantin Adolfowitsch: Taschenbuch der Mathematik, Haan-Gruiten $2016^{10}$ S. $36$}
...
Weiteres Addieren von $\varphi_1$ gibt auch keine neuen Wurzeln aus 1, da sie auch bloß vorherige Punkte ergeben.\\
...
\end{equation}
\emph{In der [Gaußschen Zahlenebene] liegen die Wurzelwerte auf einem Kreis mit Radius $\sqrt[n]{r}$, und sie haben einen konstanten Winkelabstand $\frac{2\pi}{n}$ voneinander, beginnend mit $\frac{\varphi}{n}$. Sie Bilden folglich die Eckpunkte eines regelmäßigen $n$-Ecks."} \footnote{Schmidt, Harald: Elementare Technomathematik, Amberg $2018$, S. $302$}
...
Erinnern wir uns, dass die Eckpunkte des Vielecks auf dem Einheitskreis auch als Lösungen der Gleichung $z^n=1$ beziehungsweise $z^n-1=0$ betrachtet werden konnten. Die Folge \ref{eq.faktor} des Fundamentalsatzes der Algebra besagt nun, dass 
...
$X$ ist das Produkt der Längen aller Strecken, die eine Ecke des ursprünglichen Vielecks mit den restlichen verbinden. Somit ist die Aufgabe gelöst und jenes Produkt ist gleich der Anzahl der Ecken im Vieleck.
